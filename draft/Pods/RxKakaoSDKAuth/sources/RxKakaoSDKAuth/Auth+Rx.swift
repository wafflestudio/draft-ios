...
extension Reactive where Base: Auth {
    ...
    //auth+rx용 session를 비rx session 저장소에 set 해야한다.
    //지금 문제점은
    //1. auth+rx api session 를 초기화할 장소가 마땅치 않음. (auth api session는 Auth 생성자에서 하면 됨)
    //2. auth+rx api session getter를 두는 방법이 있음.
