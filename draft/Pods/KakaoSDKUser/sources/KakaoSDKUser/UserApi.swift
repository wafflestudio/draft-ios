    ...
    public func me(propertyKeys: [String]? = nil,
        ...
        auth.responseData(.get,
                          ...
                          apiType: .KApi) { (response, data, error) in
                            ...
                            if let data = data {
                                if let user = try? SdkJSONDecoder.customIso8601Date.decode(User.self, from: data) {
                                    if useDynamicAgreement {
                                        var scopes = [String]()
                                        ...
                                        if scopes.count > 0 {
                                            AuthController.shared.authorizeWithAuthenticationSession(scopes: scopes) { (_, error) in
                                                if let error = error {
                                                    completion(nil, error)
                                                }
                                                else {
                                                    UserApi.shared.me(useDynamicAgreement:false) { (user, error) in
                                    ...
                                    //useDynamic agreement가 아닌경우
                                    //scopes가 없는경우. 일반 성공.
                                    completion(user, nil)
                                    return
                                }
                                //TODO: user decoding error
